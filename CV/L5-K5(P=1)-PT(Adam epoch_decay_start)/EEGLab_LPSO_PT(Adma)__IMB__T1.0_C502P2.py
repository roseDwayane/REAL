...
class EEGLabLOSOFeaturePytorch(object):
    ...
    def evaluate_rate_of_fif(self, group_sn):
        ...
        df_info = pd.read_csv(self.info_file, index_col=0)
        df_epochs = pd.read_csv(self.epochs_file, index_col=0)
        # df_epochs = pd.read_csv(self.epochs_file[:-4] + '_%d.csv' % (group_sn), index_col=0)
        ...
        def df_plot_(df_, dataset):
            ...
            plt.subplots_adjust(left=0.1, bottom=0.125, right=0.95, top=0.95, wspace=None, hspace=None)

            plt.plot(df0['Rate'], 'o', label=class_names[0], markersize=0.5)
            plt.plot(df1['Rate'], 'o', label=class_names[1], markersize=0.5)

            plt.legend(shadow=False, prop=text_font)
            plt.ylim([-0.1, 1.1])
            # plt.title(dataset)
            ...
            # 显示Acc & TPR
            plt.text(20, 0.5, 'Acc=%.4f' % (acc,), fontdict=text_font)
            plt.text(20, 0.5 - 0.06, 'precision=%.4f' % (precision,), fontdict=text_font)
            plt.text(20, 0.5 - 0.06 * 2, 'recall=%.4f' % (recall,), fontdict=text_font)
            plt.text(20, 0.5 - 0.06 * 3, 'Total=%d' % (len(df_),), fontdict=text_font)
            ...
            plt.figure(figsize=(2.0, 1.65), dpi=600)
            plt.subplots_adjust(left=0.1, bottom=0.125, right=0.95, top=0.95, wspace=None, hspace=None)

            plt.rcParams['font.size'] = ticks_font['size']
            plt.rcParams['font.family'] = ticks_font['family']

            plot_confusion_matrix(y_true, y_pred, self.class_names, title=None)
    ...
    def evaluate_rate_of_fif_n(self, group_sn):
        """
        分析'最新'的 self.epochs_file, 评估fif
        :param group_sn:
        :return:
        ...
        def df_plot_(df_, dataset):
            ...
            plt.figure(figsize=(2.5, 1.5), dpi=600)
            plt.subplots_adjust(left=0.15, bottom=0.175, right=0.95, top=0.95, wspace=None, hspace=None)
            ...
            # plt.ylabel('$\hat{y}$', font=label_font['family'], fontsize=label_font['size'], labelpad=0)
            plt.ylabel('p', font=label_font['family'], fontsize=label_font['size'], labelpad=1)
            plt.xlabel('sn', font=label_font['family'], fontsize=label_font['size'], labelpad=0)
            ...
            plt.figure(figsize=(2.0, 1.65), dpi=600)
            plt.subplots_adjust(left=0.215, bottom=0.225, right=0.925, top=0.95, wspace=None, hspace=None)

            plt.rcParams['font.size'] = label_font['size']
            plt.rcParams['font.family'] = label_font['family']
    ...
    def evaluate_total_rate_of_fif(self, groups):
        ...
        for group in groups:
            ...
            evaluate_dict['precision'].append(rpt['1']['precision'])
            evaluate_dict['Recall'].append(rpt['1']['recall'])

            p = rpt['1']['precision']
            r = rpt['1']['recall']
            evaluate_dict['F1'].append(2*p*r/(p+r))
    ...
    def evaluate_total_rate_of_fif_bl(self, groups):
        """
        分析'最新'的 self.epochs_file, 评估fif
        :param groups:
        :return:
        ...
        evaluate_file = self.bone_file_name + '_evaluate_bl.csv'
        ...
        for group in groups:
            ...
            df_bl = df_info[df_info['Black_List']]
            y_pred = df_bl['Pred'].to_numpy()
            y_true = df_bl['label'].to_numpy()
            ...
            evaluate_dict['precision'].append(rpt['1']['precision'])
            evaluate_dict['Recall'].append(rpt['1']['recall'])
            evaluate_dict['F1'].append(rpt['1']['f1-score'])
...
black_list_file = '../../Coteaching+/T=1.0/EEGLab_CoT+_FC1_BLA__D512_TC16V3_IMB2__C502_P2_BL.json'
