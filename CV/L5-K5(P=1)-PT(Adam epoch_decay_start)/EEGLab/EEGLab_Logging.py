...
class Monitor(object):
    def __init__(self, user=None):
        ...
        self.epochs_phase_1 = self.eeg_config['epochs_phase_1']
    ...
    def update(self, **kwarg):
        ...
        epoch = len(self.history['val_accuracy']) - 1
        if val_acc >= self.best_val_acc and epoch > self.epochs_phase_1:
    ...
    def report(self):
        ...
        if len(self.history['val_accuracy']) > self.epochs_phase_1:
            metrics_str = 'Best val_accuracy: {0:.4f} @{1:d}'.format(self.best_val_acc, self.best_epoch)
            print(metrics_str)
...
class EEGLabLogging(object):
    ...
    def _save_value(self):
        ...
        :return:
        """
        # epochs_phase_1 = self.eeg_config['epochs_phase_1']
        # pos = np.argmax(np.array(self.monitor.history['val_accuracy'][epochs_phase_1:])) + epochs_phase_1
        pos = self.monitor.best_epoch
