...
def schedule_cosine_decay_warmup_closure(lr_init=0.001, epochs=100, warmup=10):
    """
    :return:
    """
    import numpy as np
    ...
    def inner(epoch):
        ...
        lr_warmup = lr_init * epoch / warmup * _step(warmup - 1 - epoch)
        lr_cos = lr_init * _step(epoch - warmup) * (math.cos(np.pi * (epoch - warmup) / epochs) + 1.0) / 2.0

        return lr_warmup + lr_cos
