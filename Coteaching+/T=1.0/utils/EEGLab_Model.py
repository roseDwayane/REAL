...
def EEGNet_v452E(nb_classes=2, samples=251, channels=1, l2_weight=0.0001, E_dim=512):
    """
    1x1卷积扩展维度至 E_dim (from MobileNet)
    ...
    # 1x1卷积扩展维度至 E_dim (from MobileNet)
    x = Conv2D(E_dim, (1, 1), use_bias=False, kernel_regularizer=regularizers.l2(l2_weight))(x)
    x = BatchNormalization()(x)
...
if __name__ == '__main__':
    ...
    # tt_model = EEGNet_v560c(nb_classes=2, samples=251, channels=2)
    # tt_model.summary()
    tt_model = gru()
